...
async function ensureAdminSecurityRow(adminId: number) {
    ...
    await pool.query(`INSERT INTO admin_security (admin_id) VALUES (?) ON DUPLICATE KEY UPDATE admin_id = admin_id`, [adminId]);
}
...
export async function getAdminByEmail(email: string): Promise<AdminUser | null> {
    ...
    const [rows] = await pool.query(`
        ...
        LEFT JOIN admin_security s ON s.admin_id = a.id
        LEFT JOIN users u ON u.username = a.username
        WHERE s.reset_email = ? OR u.email = ? LIMIT 1
    `, [email, email]);
    return (rows as AdminUser[])[0] ?? null;
}
...
export async function getAdminNotificationEmails(): Promise<string[]> {
    ...
    const [rows] = await pool.query(`
        ...
        WHERE a.role = 'admin' AND a.is_active = 1
          AND s.reset_email IS NOT NULL AND TRIM(s.reset_email) <> ''
    `);
    return (rows as Array<{ email: string | null }>).map(r => r.email?.trim() ?? "").filter(Boolean);
}

export async function upsertAdminUser(params: { username: string; passwordHash: string; isActive: number }) {
    await ensureAdminSecurityTables();
    await pool.query(`
        INSERT INTO admins (username, password_hash, role, is_active)
        VALUES (?, ?, 'admin', ?)
        ON DUPLICATE KEY UPDATE password_hash = VALUES(password_hash), role = 'admin', is_active = VALUES(is_active)
...
export async function getAdminSecurityState(adminId: number): Promise<AdminSecurityState> {
    ...
    const [rows] = await pool.query(`SELECT reset_email, password_changed_at, reset_required, last_reset_email_sent_at FROM admin_security WHERE admin_id = ? LIMIT 1`, [adminId]);
    return (rows as AdminSecurityState[])[0];
}

export async function setAdminResetEmail(adminId: number, email: string) {
    await ensureAdminSecurityRow(adminId);
    await pool.query(`UPDATE admin_security SET reset_email = ? WHERE admin_id = ?`, [email, adminId]);
}

export async function markResetEmailSent(adminId: number) {
    await ensureAdminSecurityRow(adminId);
    await pool.query(`UPDATE admin_security SET last_reset_email_sent_at = NOW(), reset_required = 1 WHERE admin_id = ?`, [adminId]);
}
...
export async function setAdminTemporaryPassword(adminId: number, passwordHash: string) {
    ...
    await pool.query(`UPDATE admin_security SET reset_required = 1, last_reset_email_sent_at = NOW() WHERE admin_id = ?`, [adminId]);
}
...
export async function createPasswordResetToken(adminId: number, ttlMinutes = 60) {
    ...
    await pool.query(`INSERT INTO admin_password_reset_tokens (admin_id, token_hash, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL ? MINUTE))`, [adminId, tokenHash, ttlMinutes]);
    return rawToken;
}
...
export async function consumePasswordResetToken(rawToken: string): Promise<AdminResetToken | null> {
    await ensureAdminSecurityTables();
    const tokenHash = hashToken(rawToken);
    const [rows] = await pool.query(`SELECT id, admin_id, token_hash, expires_at, used_at, created_at FROM admin_password_reset_tokens WHERE token_hash = ? LIMIT 1`, [tokenHash]);
    const token = (rows as AdminResetToken[])[0];
    if (!token || token.used_at || new Date(token.expires_at).getTime() < Date.now()) return null;
    await pool.query(`UPDATE admin_password_reset_tokens SET used_at = NOW() WHERE id = ?`, [token.id]);
    return token;
}
...
export async function createLoginVerificationChallenge(params: {
    ...
    `, [params.userId, params.userType, params.username, params.role, params.email, codeHash, challengeHash, ttlMinutes]);
...
export async function consumeLoginVerificationChallenge(rawChallengeToken: string, code: string): Promise<LoginVerificationChallenge | null> {
    ...
    const [rows] = await pool.query(`
        ...
        FROM admin_login_verification_challenges WHERE challenge_hash = ? LIMIT 1
    `, [challengeHash]);
    const challenge = (rows as LoginVerificationChallenge[])[0];
    if (!challenge || challenge.consumed_at || new Date(challenge.expires_at).getTime() < Date.now() || challenge.code_hash !== codeHash) return null;
    await pool.query(`UPDATE admin_login_verification_challenges SET consumed_at = NOW() WHERE id = ?`, [challenge.id]);
    return challenge;
}
...
export async function getActiveLoginVerificationChallenge(rawChallengeToken: string): Promise<LoginVerificationChallenge | null> {
    ...
    `, [challengeHash]);
    const challenge = (rows as LoginVerificationChallenge[])[0];
    if (!challenge || challenge.consumed_at || new Date(challenge.expires_at).getTime() < Date.now()) return null;
    return challenge;
}
