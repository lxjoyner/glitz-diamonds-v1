...
export default async function PublicInvoicePage({
    ...
    const isPaid = balance === 0 || invoice.display_status === "paid";
            ...
            <div className="mx-auto max-w-4xl rounded-2xl bg-white p-6 shadow-lg sm:p-10">
                {query.payment === "success" ? (
                    <div className="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
                        Payment submitted successfully. Your invoice status will update after Stripe confirms the payment.
                    </div>
                ) : null}
                {query.payment === "cancelled" ? (
                    <div className="mb-6 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                        Payment was cancelled. No payment was applied to this invoice.
                    </div>
                ) : null}
                ...
                <div className="grid gap-8 py-8 md:grid-cols-2">
                    ...
                    <div className="md:text-right">
                        ...
                        {isPaid ? (
                            <div className="mt-4 inline-block rounded-full bg-emerald-100 px-4 py-2 text-sm font-semibold text-emerald-800">Paid in full</div>
                        ) : (
                            <div className="ml-auto max-w-xs"><InvoicePayButton token={token} /></div>
                        )}
                    </div>
                </div>
                ...
                <div className="ml-auto mt-8 max-w-sm space-y-3 text-sm">
                    ...
                    <div className="flex justify-between"><span>Paid</span><strong>{money(Number(invoice.amount_paid_cents))}</strong></div>
                    ...
                    <div className="mt-8">
                        ...
                        <div className="mt-3 overflow-x-auto rounded-xl border border-slate-200">
                            <table className="w-full min-w-[520px] text-sm">
                                <thead className="bg-slate-50 text-left"><tr><th className="px-4 py-3">Date</th><th className="px-4 py-3">Provider</th><th className="px-4 py-3 text-right">Amount</th></tr></thead>
                                <tbody>{invoice.payments.map((payment) => <tr key={payment.id} className="border-t border-slate-100"><td className="px-4 py-3">{payment.paid_at ? new Date(payment.paid_at).toLocaleString() : "Paid"}</td><td className="px-4 py-3 capitalize">{payment.provider}</td><td className="px-4 py-3 text-right font-semibold">{money(Number(payment.amount_cents))}</td></tr>)}</tbody>
                            </table>
                        </div>
                    </div>
                ) : null}
