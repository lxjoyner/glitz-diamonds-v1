...
export default function EditRecurringInvoicePage() {
    ...
    useEffect(() => {
        async function load() {
            ...
            const recurring = recurringData.recurringInvoice as RecurringInvoice;
            setMemberId(String(recurring.member_id));
            setStatus(recurring.status === "draft" ? "draft" : "active");
    ...
    async function save() {
        ...
        try {
            ...
            router.push(`/admin/invoices/recurring/${params.id}`);
            router.refresh();
        } catch (error) {
            setMessage(error instanceof Error ? error.message : "Failed to update recurring invoice.");
        } finally {
            setSaving(false);
        }
    }
        ...
        <main className="min-h-screen bg-[#f7f9fc] px-4 py-8 text-slate-950 sm:px-8">
            <div className="mx-auto max-w-[1500px]">
                <header className="mb-6 flex min-h-[108px] flex-wrap items-center justify-between gap-5 px-4 py-5 sm:px-6 sm:py-6">
                    <h1 className="text-4xl font-bold tracking-tight leading-tight text-white">Edit recurring invoice</h1>
                ...
                <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/50">
                    <div className="grid gap-8 p-6 md:grid-cols-2 md:p-8">
                        ...
                        <div className="space-y-4">
                            ...
                            <label className="grid grid-cols-[140px_1fr] items-center gap-3 text-sm"><span className="font-semibold">First invoice</span><InvoiceDatePicker value={firstInvoiceDate} onChange={setFirstInvoiceDate} ariaLabel="First invoice date" /></label>
                            <label className="grid grid-cols-[140px_1fr] items-center gap-3 text-sm"><span className="font-semibold">Next invoice</span><InvoiceDatePicker value={nextInvoiceDate} onChange={setNextInvoiceDate} ariaLabel="Next invoice date" /></label>
                        </div>
                    </div>
