                    ...
                    <div className="space-y-5">
                        ...
                        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            ...
                            <div className="mx-auto max-w-4xl space-y-4 text-sm">
                                ...
                                <div className="flex flex-wrap items-center gap-3"><span>Create first invoice on</span><div className="min-w-[220px]"><InvoiceDatePicker label="First invoice" value={firstInvoiceDate} onChange={setFirstInvoiceDate} compact /></div><span>and end</span><select value={endMode} onChange={(e) => setEndMode(e.target.value as EndMode)} className="rounded-lg border border-slate-300 px-3 py-2"><option value="after">After</option><option value="on">On</option><option value="never">Never</option></select>{endMode === "after" ? <><input type="number" min="1" value={endValue} onChange={(e) => setEndValue(Math.max(1, Number(e.target.value) || 1))} className="w-20 rounded-lg border border-slate-300 px-3 py-2"/><span>invoice(s)</span></> : null}{endMode === "on" ? <div className="min-w-[220px]"><InvoiceDatePicker label="End date" value={endDate} onChange={setEndDate} compact /></div> : null}</div>
                        ...
                        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                            ...
                            <div className="grid grid-cols-[1fr_100px_120px_120px] gap-3 border-b border-slate-200 px-6 py-4 text-sm"><div><p>Dues</p><p className="text-xs text-slate-500">{settings.business_name || "Glitz Of Diamonds"}</p></div><span>1</span><span className="text-right">{money(invoice.amount_cents)}</span><span className="text-right">{money(invoice.amount_cents)}</span></div>
